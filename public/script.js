...
// Function to fetch data from ThingSpeak and refresh charts
async function fetchDataAndRefreshCharts() {
  // Your ThingSpeak channel ID and API key
  const channelID = "2369829";
  const apiKey = "75U1406FS2G9BFG8";

  // URLs for fetching data from ThingSpeak
  ...
  try {
    // Fetch data for temperature, humidity, pressure, etc.
    ...
    // Extract timestamp and values for temperature
    ...
    // Extract timestamp and values for humidity
    ...
    // Extract timestamp and values for pressure
    ...
    tempChart.data.labels = timestamps.map((ts) => ts.format("HH:mm:ss"));
    tempChart.data.datasets[0].data = valuesField1;
    tempChart.data.datasets[1].data = valuesField4;
    tempChart.update();

    // Update humidity chart
    humidityChart.data.labels = timestampsHumidity.map((ts) =>
      ts.format("HH:mm:ss")
    );
    humidityChart.data.datasets[0].data = valuesHumidityField2;
    humidityChart.data.datasets[1].data = valuesHumidityField5;
    humidityChart.update();

    // Update pressure chart
    pressureChart.data.labels = timestampsPressure.map((ts) =>
      ts.format("HH:mm:ss")
    );
    pressureChart.data.datasets[0].data = valuesPressureField3;
    pressureChart.data.datasets[1].data = valuesPressureField6;
    pressureChart.update();

    // ... (add similar update logic for additional charts if needed)
...
// Call the fetchDataAndRefreshCharts function every 20 seconds (adjust as needed)
setInterval(fetchDataAndRefreshCharts, 20 * 1000); // 20 seconds in milliseconds

// Call the fetchDataAndRefreshCharts function when the page loads
document.addEventListener("DOMContentLoaded", fetchDataAndRefreshCharts);
